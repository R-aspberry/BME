...
import { getMyProjects, getProjects } from '../../services/projects'
...
export default function EmployeeProjects() {
  ...
  useEffect(() => {
    (view === 'mine' ? getMyProjects() : getProjects()).then(setProjects).catch(() => setProjects([]))
  }, [view])
  ...
  return <div className="employee-content aaib-animate-fade">
    ...
    <div className="employee-project-grid">{filtered.map(project => <Link to={`/employee/projects/${project.prj_ID}`} className="aaib-panel employee-project-card" key={project.prj_ID}><div className="employee-project-card-top"><span className="employee-project-id">PRJ-{String(project.prj_ID).padStart(3, '0')}</span><span className="aaib-badge aaib-badge-neutral">{project.status || 'Open'}</span></div><h2>{project.project_Name || 'Untitled project'}</h2><p>{project.description || 'No project description is available.'}</p><div className="employee-project-meta"><span>{project.start_date ? new Date(project.start_date).toLocaleDateString() : 'Start date TBD'}</span><span>{project.flag || 'Bank initiative'}</span></div></Link>)}</div>
