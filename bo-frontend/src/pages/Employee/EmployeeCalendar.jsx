...
export default function EmployeeCalendar() {
  ...
  const visibleEvents = events.filter(event => event.month === monthCode)
  ...
  return <div className="employee-content aaib-animate-fade"><div className="employee-page-heading"><div><p className="employee-eyebrow">YOUR SCHEDULE</p><h1>Calendar</h1><p>Keep project milestones and personal reminders in view.</p></div><button className="aaib-btn aaib-btn-primary" onClick={addReminder}>+ Add reminder</button></div><div className="employee-calendar-layout"><section className="aaib-panel employee-calendar"><div className="employee-calendar-top"><h2>{label}</h2><div><button className="calendar-arrow" onClick={() => setMonthOffset(value => value - 1)}>‹</button><button className="calendar-arrow" onClick={() => setMonthOffset(value => value + 1)}>›</button></div></div><div className="calendar-weekdays">{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-days">{Array.from({ length: firstDay }, (_, index) => <span className="calendar-empty-day" key={`empty-${index}`} />)}{Array.from({ length: days }, (_, index) => <span className={visibleEvents.some(event => Number(event.date) === index + 1) ? 'has-event' : ''} key={index}>{index + 1}</span>)}</div></section><section className="employee-upcoming"><p className="employee-eyebrow">UP NEXT</p><h2>Upcoming events</h2>{visibleEvents.length ? visibleEvents.map(event => <div className="employee-event" key={event.id}><div className={`employee-event-date ${event.tone}`}><strong>{event.date}</strong><span>{event.month}</span></div><div><strong>{event.title}</strong><p>{event.detail}</p></div></div>) : <div className="employee-calendar-empty">No events scheduled for this month.</div>}</section></div></div>
}
