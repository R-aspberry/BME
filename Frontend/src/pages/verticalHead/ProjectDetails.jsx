...
const getProjectId = (project) =>
  getValue(project, ['prj_ID', 'projectId', 'id'], null);
...
const getDescription = (project) =>
  getValue(project, ['description', 'Description'], '');

const getStatus = (project) =>
  getValue(project, ['status', 'Status'], 'Unknown');

const getFlag = (project) =>
  getValue(project, ['flag', 'Flag'], null);

const getMvp = (project) =>
  getValue(project, ['mvp', 'MVP'], 'Not provided');

const getBudget = (project) =>
  getValue(project, ['budget', 'Budget'], null);

const getStartDate = (project) =>
  getValue(
    project,
    ['start_date', 'startDate', 'Start_date', 'Start_Date'],
    null
  );
...
const getBrd = (project) =>
  getValue(project, ['brd', 'BRD'], null);

const getBusinessOwnerId = (project) =>
  getValue(
    project,
    ['bO_ID', 'bo_ID', 'BO_ID', 'boId'],
    null
  );

const getBusinessProblem = (project) =>
  getValue(project, ['businessProblem', 'BusinessProblem'], '');

const getBusinessObjective = (project) =>
  getValue(project, ['businessObjective', 'BusinessObjective'], '');

const getExpectedOutcome = (project) =>
  getValue(project, ['expectedOutcome', 'ExpectedOutcome'], '');

const getMustHave = (project) =>
  getValue(project, ['mustHave', 'MustHave'], '');

const getNiceToHave = (project) =>
  getValue(project, ['niceToHave', 'NiceToHave'], '');

const getExpectedBenefits = (project) =>
  getValue(project, ['expectedBenefits', 'ExpectedBenefits'], '');

const getSuccessMetrics = (project) =>
  getValue(project, ['successMetrics', 'SuccessMetrics'], '');
...
export default function VerticalHeadProjectDetails() {
  const { id } = useParams();
  ...
  const loadProject = () => {
  // "create" is not a valid project ID.
  // Send the user back to the VH projects page instead of calling the API.
  if (String(id).toLowerCase() === 'create') {
    navigate('/vertical-head/projects', { replace: true });
    return;
  }
  ...
  const flagType = useMemo(() => getFlagType(flag), [flag]);

  const projectName = getProjectName(project);
  const projectId = getProjectId(project);
  const description = getDescription(project);

  const businessProblem = getBusinessProblem(project);
  const businessObjective = getBusinessObjective(project);
  const expectedOutcome = getExpectedOutcome(project);
  const mustHave = getMustHave(project);
  const niceToHave = getNiceToHave(project);
  const expectedBenefits = getExpectedBenefits(project);
  const successMetrics = getSuccessMetrics(project);
  ...
  const hasOutcomes = expectedBenefits || successMetrics;
      ...
      <div className="po-project-detail-loading-page">
        ...
        <style>{`
          ...
          .po-project-detail-loading-icon svg {
            animation: poProjectDetailSpin 1s linear infinite;
          }

          @keyframes poProjectDetailSpin {
    ...
    return (
      <div className="po-project-detail-loading-page">
        <div className="po-project-detail-loading-card">
          ...
          <span>
            {errorMessage || 'The requested project could not be loaded.'}
          </span>
          <div className="po-project-error-actions">
            <button
              type="button"
              className="aaib-btn aaib-btn-secondary"
              onClick={() => navigate('/vertical-head/projects')}
            >
...
>
  ...
  <span className="po-nav-text">Resource Requests</span>
          ...
          <div className="po-content">
            ...
            <section className="po-project-hero">
              <div className="po-project-hero-left">
                <div className="po-project-hero-copy">
              ...
              <div className="po-project-hero-right">
                <span className={`po-project-status ${statusType}`}>
                  {status}
                </span>

                {flag && (
                  <span className={`po-project-flag ${flagType}`}>
            ...
            <div className="po-project-details-grid">
              <div className="po-project-main-column">
                <section className="po-project-card">
                  ...
                  <p className="po-project-description">
                    {description || (
                      <span className="po-muted-content">
                        No project description has been provided.
                                ...
                                <section className="po-project-card po-mvp-only-card">
                  <div className="po-project-section-header">
                  ...
                  <div className="po-mvp-scope-content">
                    <div className="po-mvp-scope-label">
                    ...
                    <p>
                      {mvp || (
                        <span className="po-muted-content">
                          No MVP scope has been provided.
                        </span>
                      )}
                    </p>
                  </div>
                ...
                {/* {hasRequirements ? (
                  <section className="po-project-card">
                    ...
                    <div className="po-business-info-grid">
                      <div className="po-business-info">
                        <span>BUSINESS PROBLEM</span>
                        <p>{businessProblem || 'Not provided'}</p>
                      </div>

                      <div className="po-business-info">
                        <span>BUSINESS OBJECTIVE</span>
                        <p>{businessObjective || 'Not provided'}</p>
                      </div>

                      <div className="po-business-info">
                        <span>EXPECTED OUTCOME</span>
                        <p>{expectedOutcome || 'Not provided'}</p>
                      </div>

                      <div className="po-business-info">
                        <span>MVP SCOPE</span>
                        <p>{mvp || 'Not provided'}</p>
                      </div>
                    </div>
                  </section>
                ) : (
                  <section className="po-project-card po-empty-data-card">
                    <div className="po-project-section-header">
                      <div className="po-project-section-icon gold">
                        <Icon name="briefcase" size={18} />
                      </div>
                    ...
                    <p>
                      No additional business requirement fields are included
                      in the current project response.
                    </p>
                  </section>
                )} */}
                ...
                <section className="po-project-card">
                  <div className="po-project-section-header">
                    ...
                    <div>
                      ...
                      <h2>Must-Have & Nice-to-Have</h2>
                    </div>
                  </div>

                  <div className="po-scope-grid">
                    <div className="po-scope-card must">
                      <div>
                        <span>MUST-HAVE</span>
                        <strong>Required</strong>
                      </div>
                      <p>
                        {mustHave || 'No must-have requirements provided.'}
                      </p>
                    </div>

                    <div className="po-scope-card nice">
                      <div>
                        <span>NICE-TO-HAVE</span>
                        <strong>Optional</strong>
                      </div>
                      <p>
                        {niceToHave || 'No nice-to-have requirements provided.'}
                      </p>
                    </div>
                  </div>
                </section>
                ...
                <section className="po-project-card">
                  <div className="po-project-section-header">
                    ...
                    <div>
                      ...
                      <h2>Benefits & Success Metrics</h2>
                    </div>
                  </div>
                  ...
                  <div className="po-business-info-grid">
                    <div className="po-business-info">
                      ...
                      <p>{expectedBenefits || 'Not provided'}</p>
                    </div>

                    <div className="po-business-info">
                      <span>SUCCESS METRICS</span>
                      <p>{successMetrics || 'Not provided'}</p>
                    </div>
                  </div>

                  {/* {!hasOutcomes && (
                    <div className="po-outcomes-note">
                      No additional outcome information is included in the
                ...
                <section className="po-project-card">
                  ...
                  <button
                    ...
                    onClick={() => navigate('/vertical-head/calendar')}
                  >
                    Open Calendar
                ...
                {/* <section className="po-project-card po-resource-card">
                  ...
                  <div className="po-resource-planning-content">
                    <div className="po-resource-planning-message">
                      <strong>Continue with resource planning</strong>
                      <p>
                        Define the required number of employees by department,
                        then follow the resource allocation workflow with the
                        respective Vertical Heads.
                      </p>
                    </div>
                    ...
                    <button
                      type="button"
                      className="aaib-btn aaib-btn-secondary"
                      onClick={() => navigate('/vertical-head/resource-requests')}
                    >
              ...
              <aside className="po-project-side-column">
                <section className="po-project-side-card">
                  ...
                  <div className="po-owner-box">
                    ...
                    <small>
                      The current project API response does not include the
                      Business Owner name.
                    </small>
                  </div>
                </section>
                    ...
                    brdIsOpenable ? (
                      ...
                      <div className="po-brd-card unavailable">
                        ...
                        <div>
                          <strong>{brdLabel}</strong>
                          <span>
                            The document is listed, but the current API
                ...
                <section className="po-project-side-card">
                  <div className="po-side-section-header">
                    <div className={`po-side-section-icon ${flag ? 'danger' : ''}`}>
                      <Icon name={flag ? 'flag' : 'check'} size={17} />
                    </div>
                    <div>
                  ...
                  {flag ? (
                    <div className="po-active-flag">
                      <span className={`po-active-flag-badge ${flagType}`}>
                        Active
                      </span>
                      <strong>{flag}</strong>
                      <p>This project has an active flag.</p>
                    </div>
                  ) : (
      ...
      <style>{`
        ...
        .po-business-info > span,
        .po-scope-card > div span {
          display: block;
          color: var(--aaib-text-muted);
          font-size: 7px;
          font-weight: 800;
        ...
        .po-project-section-header > div:last-child,
        .po-side-section-header > div:last-child {
          min-width: 0;
        }
        ...
        .po-project-information-item.wide {
          grid-column: span 1;
        }
        ...
        .po-scope-card.nice {
          border-top: 3px solid var(--aaib-accent);
        }
        ...
        .po-scope-card.nice > div span {
          color: #99751f;
        }

        .po-scope-card > div strong {
          color: var(--aaib-text-muted);
          font-size: 8px;
        }
        ...
        .po-active-flag-badge.critical {
          color: var(--aaib-danger);
          background: rgba(201,58,58,.09);
        }

        .po-active-flag-badge.attention {
          color: var(--aaib-warning);
          background: var(--aaib-warning-soft);
        }

        .po-active-flag-badge.low {
          color: var(--aaib-success);
          background: var(--aaib-success-soft);
        }

        .po-active-flag-badge.info {
          color: #2c6ba4;
          background: #eaf2ff;
        }
        ...
        .po-project-detail-actions .aaib-btn {
          min-height: 43px;
        }
